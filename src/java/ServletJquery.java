...
public class ServletJquery extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
             ...
             con.setConsult("select * from paises");
             ArrayList lista=new ArrayList();
             
            try {
                while (con.getResult().next()){
                    paises pai= new paises();
                    pai.setPais_id(con.getResult().getInt("pais_id"));
                    pai.setNombre(con.getResult().getString("nombre"));
                    pai.setEstado(con.getResult().getString("estado"));
                    lista.add(pai);
